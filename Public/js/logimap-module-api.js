...
//var server_ext = "";
//
//$.getJSON("lbsconfig.json", function(json) {
//    server_url = json.server_url;
//    server_ext = json.server_ext;
//});
let server_url = MAP_SERVER_URL.server_url;
let server_ext = MAP_SERVER_URL.server_ext;
...
class MapModule 
{
	constructor() 
    {
        ...
        console.log("맵 타일 서버  server_url "+ server_url );
        console.log("MAP SERVER EXT  server_url "+ server_ext );
	}
    ...
    setImageMask = function (bCheck) { var nResult = Module.ccall('setImageMask', 'number', ['number'], [bCheck]); }; //Only ALOA
    setImageMaskLabel = function (bCheck) { var nResult = Module.ccall('setImageMaskLabel', 'number', ['number'], [bCheck]); }; //Only ALOA
    setLineVisibleLimit = function (bCheck) { var nResult = Module.ccall('setLineVisibleLimit', 'number', ['number'], [bCheck]); }; //Only ALOA
    setMoveDrawUserImage = function (bCheck) { var nResult = Module.ccall('setMoveDrawUserImage', 'number', ['number'], [bCheck]); }; //Only ALOA
    setMoveDrawUserText = function (bCheck) { var nResult = Module.ccall('setMoveDrawUserText', 'number', ['number'], [bCheck]); }; //Only ALOA
    setMoveDrawUserPoly = function (bCheck) { var nResult = Module.ccall('setMoveDrawUserPoly', 'number', ['number'], [bCheck]); }; //Only ALOA
    ...
    //-----------------------------------------------------------------------------------------------------------------
    //* Event Handler
    mousedoubleclickEventHanlder = function (mouseX, mouseY)
    { 
        if((this.module != null) && (this.loadedWasm == true))
        {
